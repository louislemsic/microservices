...
async function bootstrap() {
  ...
  logger.log(`Posts service config - NAME: ${serviceName}, VERSION: ${serviceVersion} (API: ${apiVersion}), PORT: ${port}, REG_KEY: ${regKey ? `${regKey.slice(0, 8)}...${regKey.slice(-4)}` : 'undefined'}`);

  app.setGlobalPrefix(serviceName);

  await app.listen(port);
  logger.log(`🚀 Posts service is running on: http://localhost:${port}/${serviceName}/${apiVersion}`);
  ...
  async function registerWithGateway() {
    ...
    const registration = {
      ...
      timestamp: new Date(),
      metadata: {
        description: packageJson.description || 'Posts service for blog post management',
        tags: ['atlas', 'microservice', serviceName, 'content'],
